...
public class LC321 {
    public int[] maxNumber(int[] nums1, int[] nums2, int k) {
        ...
        if (len1 + len2 < k) return result;//bad case
        else if (len1 + len2 == k) result = mergeTwoArrays(nums1, nums2, k);//edge case
    ...
    private int[] mergeTwoArrays(int[] nums1, int[] nums2, int k) {
        ...
        while (idx < k) {
            ...
            if (compareTwoArrays(nums1, idx1, nums2, idx2)) result[idx] = nums1[idx1++];
